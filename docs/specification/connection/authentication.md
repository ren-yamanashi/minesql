...
- アカウント管理ステートメント (`CREATE USER` / `ALTER USER` / `DROP USER`) と `GRANT` / `REVOKE` の実行 (`mysql.user` への行の書き込み) -> 実行の段階 (`executor/`) で行い、認証はその結果を読むだけ ([ADR-0017](../adr/0017.アカウント管理ステートメントを対象に含める.md))
- アカウントのロック、パスワードの期限、`offline_mode`、TLS 要件 (`require_secure_transport` とアカウントの `ssl_type`) の検査 -> 対象外 (アカウント管理・サーバーモード・TLS の機能に付随するため)
- ステートメントごとの権限の検査 -> プリペアの入口 (名前解決の前) で行う ([ADR-0018](../adr/0018.権限は全体権限だけを持つ.md))
- ACL キャッシュ (user@host のパターン照合) -> 持たない (ホストは `%` のみ)
...
- アカウント照合 (Account_verification_handler): 資格情報を「既定スキーマ \0 ユーザー名 \0 パスワード (または計算値)」に分解し、アカウント情報を取り出して照合する
  ...
  - アカウント情報はシステムスキーマ `mysql` の `user` 表に置き、内部セッションで検索して取る (minesql の行は認証文字列と全体権限を持ち、MySQL はさらにプラグイン名、ロック状態、パスワードの期限、TLS の要件を持つ)
    - この表と初期アカウントは起動時の bootstrap で作る ([ADR-0014](../adr/0014.スキーマを持つ.md))
    - アカウントはアカウント管理ステートメント (`CREATE USER` / `ALTER USER` / `DROP USER`) で増減し、ホストは `%` のみ (user@host のパターン照合は行わない、[ADR-0017](../adr/0017.アカウント管理ステートメントを対象に含める.md))
...
- SHA256 パスワードキャッシュ: サーバーに 1 つ、`SHA256_MEMORY` の照合に使う値 (パスワードの SHA256 の SHA256) を利用者ごとに保持する
  ...
  - 資格情報の変更 (`ALTER USER ... IDENTIFIED BY`) とアカウントの削除 (`DROP USER`) で、その利用者のエントリが消える ([ADR-0017](../adr/0017.アカウント管理ステートメントを対象に含める.md))
...
- 内部セッションの実行ユーザー (security context): 照合の間はシステムユーザー (`mysql.session`@`localhost`) として動き、成功したら認証した利用者に切り替え、既定スキーマの指定があればそれも設定し、同じ行の全体権限を実行ユーザーに載せる ([ADR-0018](../adr/0018.権限は全体権限だけを持つ.md))
...
| コネクションハンドラー: 認証ハンドラとアカウント照合 (この文書) | メカニズムの進行 (salt の生成と応答の受け取り)、資格情報の分解、照合の計算、SHA256 パスワードキャッシュの更新、全体権限の実行ユーザーへの載せ込み |
| 内部セッション | システムユーザーとしての `mysql.user` の SELECT の実行、実行ユーザーの利用者への切り替えと既定スキーマの設定 |
| プリペア | ステートメントごとの権限の検査 (名前解決の前) |
| 実行の段階 (`executor/`) | アカウント管理ステートメントと `GRANT` / `REVOKE` の実行 (`mysql.user` への行の書き込みとキャッシュのエントリの削除) |
...
[要件](#要件) のうち、minesql が満たすものと満たさないもの。判断の理由は各 ADR を参照

| 要件 | minesql | 備考 |
| --- | --- | --- |
| WL#9271 機能要件: `sha256_password` のアカウントへの `PLAIN` 認証 | 該当なし | アカウントの認証プラグインは `caching_sha2_password` のみで、`sha256_password` のアカウントは存在しない ([ADR-0015](../adr/0015.認証の方式.md))。`caching_sha2_password` のアカウントへの `PLAIN` 認証 (新しい接続時と、セッションのリセット後の再認証) は満たす |
| WL#9271 認証全般: ハッシュが一致しなければ失敗、いずれの失敗条件にも該当しなければ成功 | 満たす | 検査項目はパスワードの一致だけ ([ADR-0015](../adr/0015.認証の方式.md)) |
| WL#9271 認証全般: ロック、オフラインモード、パスワードの期限、`ssl/tls` オプションによる失敗と、期限切れの `Notice` | 満たさない | [担わないこと](#担わないこと) |
| WL#10992 認証資格情報のキャッシュ: 安全な接続上の `PLAIN` の成功でキャッシュに保存する、一致するエントリがあれば `PLAIN` は成功する、`SHA256_MEMORY` はキャッシュのエントリと一致すれば成功する、`SHA256_MEMORY` は安全でない接続でも安全な接続でも動作する | 満たす | [構成要素](#構成要素) の SHA256 パスワードキャッシュ。安全な接続は TLS ([ADR-0016](../adr/0016.TLS接続を実装する.md)) または Unix ソケット |
| WL#10992 認証資格情報のキャッシュ: 無効化が機能しない場合の失敗 | 該当なし | 無効化を止める手段 (MySQL の `mysqlx_cache_cleaner=0` に相当するもの) を持たない |
| WL#10992 認証資格情報のキャッシュ: 安全でない接続で tls_option のあるアカウントの失敗 | 満たさない | アカウントの TLS 要件を検査しない ([担わないこと](#担わないこと)) |
| WL#10992 プロトコル実装: 1 回の失敗で接続を閉じない、複数回の失敗で閉じる、リセット時に失敗回数を戻す | 満たす | 失敗は 1 セッションにつき 3 回まで。3 回目の失敗の `Error` は FATAL になり、サーバーは認証を打ち切って接続を閉じる (途中で別のメカニズムに切り替えて試すことはできる)。セッションを閉じた / リセットした後は新しいセッションになるため、回数は 0 から数え直す |
| WL#10992 エッジケース: SQL からキャッシュにアクセスできない、キャッシュを永続化しない | 満たす | キャッシュはサーバーのメモリ上にだけ持ち、システム表にも置かない |
| WL#10992 キャッシュの無効化: `DROP USER` / `ALTER USER` | 満たす | アカウント管理ステートメントの実行時に、その利用者のエントリを消す ([ADR-0017](../adr/0017.アカウント管理ステートメントを対象に含める.md)) |
| WL#10992 キャッシュの無効化: `SET PASSWORD` / `RENAME USER` / `IDENTIFIED` を伴う `GRANT` / `FLUSH PRIVILEGES` | 該当なし | これらのステートメントを持たない ([ADR-0017](../adr/0017.アカウント管理ステートメントを対象に含める.md))。入れるときは同じく実行時にエントリを消す |
| WL#10992 libmysqlxclient | 該当なし | クライアントは作らず、公式のクライアントを使う ([ADR-0001](../adr/0001.実装対象はXProtocolに従うサーバー.md)) |
| WL#9591 プラグイン: サーバー側の `caching_sha2_password`、`authentication_string` へのハッシュの保存 (`sha256_password` と同様の手法)、高コストな認証 (平文) | 満たす | 認証プラグインの枠組みは持たず、この 1 方式だけを持つ。認証文字列は `$A$005$` + 20 バイトの salt + ダイジェスト (SHA256 を 5000 回反復)。高コストな認証にあたるのが `PLAIN` |
| WL#9591 プラグイン: クライアント側の `caching_sha2_password`、`MYSQL_OPT_GET_SERVER_PUBLIC_KEY`、RSA 鍵ペアによるパスワードの交換 | 該当なし | classic protocol のクライアントライブラリは対象外 ([ADR-0001](../adr/0001.実装対象はXProtocolに従うサーバー.md))。高コストな認証の接続は TLS または Unix ソケットに限る |
| WL#9591 プラグイン: メモリ上のキャッシュと高速認証 (スクランブル) | 該当なし | classic protocol の高速認証は持たない。同じ役割 (SHA256(SHA256(password)) のキャッシュとチャレンジ / レスポンス) は X Plugin 側の SHA256 パスワードキャッシュと `SHA256_MEMORY` が担う (MySQL でも 2 つのキャッシュは独立) |
| WL#9591 監査クラスと監査イベント、`sha2_cache_cleaner` | 該当なし | 監査の枠組みを持たず、キャッシュのエントリはアカウント管理ステートメントの実行時に直接消す ([ADR-0017](../adr/0017.アカウント管理ステートメントを対象に含める.md)) |
| WL#9591 非機能要件: アップグレード時にアカウントを変更しない | 該当なし | アップグレードを持たない |
