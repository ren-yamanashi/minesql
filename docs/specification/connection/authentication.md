...
- アカウント管理ステートメント (`CREATE USER` / `ALTER USER` / `DROP USER`) と `GRANT` / `REVOKE` の実行 (`mysql.user` への行の書き込み) -> 実行の段階 (`executor/`) で行い、認証はその結果を読むだけ ([SDR-0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md))
- アカウントのロック、パスワードの期限、`offline_mode`、TLS 要件 (`require_secure_transport` とアカウントの `ssl_type`) の検査 -> 対象外 (アカウント管理・サーバーモード・TLS の機能に付随するため)
- ステートメントごとの権限の検査 -> プリペアの入口 (名前解決の前) で行う ([SDR-0018](../sdr/0018.権限は全体権限だけを持つ.md))
...
- 内部セッションの実行ユーザー (security context): 照合の間はシステムユーザー (`mysql.session`@`localhost`) として動き、成功したら認証した利用者に切り替え、既定スキーマの指定があればそれも設定し、同じ行の全体権限を実行ユーザーに載せる ([SDR-0018](../sdr/0018.権限は全体権限だけを持つ.md))
...
| WL | 種類 | 要件 | minesql | SDR |
| --- | --- | --- | --- | --- |
| WL#9271 | 機能要件 | `sha256_password` のアカウントへの `PLAIN` 認証 | 該当なし (`caching_sha2_password` のアカウントへの `PLAIN` 認証は満たす) | [0015](../sdr/0015.認証の方式.md) |
| WL#9271 | 認証全般 | ハッシュが一致しなければ失敗、いずれの失敗条件にも該当しなければ成功 | 満たす | [0015](../sdr/0015.認証の方式.md) |
| WL#9271 | 認証全般 | ロック、オフラインモード、パスワードの期限、`ssl/tls` オプションによる失敗と、期限切れの `Notice` | 満たさない | [0015](../sdr/0015.認証の方式.md) |
| WL#10992 | 認証資格情報のキャッシュ | 安全な接続上の `PLAIN` の成功でキャッシュに保存する、一致するエントリがあれば `PLAIN` は成功する、`SHA256_MEMORY` はキャッシュのエントリと一致すれば成功する、`SHA256_MEMORY` は安全でない接続でも安全な接続でも動作する | 満たす | [0015](../sdr/0015.認証の方式.md)、[0016](../sdr/0016.TLS接続を実装する.md) |
| WL#10992 | 認証資格情報のキャッシュ | 無効化が機能しない場合の失敗 | 該当なし (無効化を止める手段を持たない) |  |
| WL#10992 | 認証資格情報のキャッシュ | 安全でない接続で tls_option のあるアカウントの失敗 | 満たさない | [0015](../sdr/0015.認証の方式.md) |
| WL#10992 | プロトコル実装 | 1 回の失敗で接続を閉じない、複数回の失敗で閉じる、リセット時に失敗回数を戻す | 満たす |  |
| WL#10992 | エッジケース | SQL からキャッシュにアクセスできない、キャッシュを永続化しない | 満たす |  |
| WL#10992 | キャッシュの無効化 | `DROP USER` / `ALTER USER` | 満たす | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| WL#10992 | キャッシュの無効化 | `SET PASSWORD` / `RENAME USER` / `IDENTIFIED` を伴う `GRANT` / `FLUSH PRIVILEGES` | 該当なし | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| WL#10992 | libmysqlxclient | `SHA256_MEMORY` への対応 | 該当なし | [0001](../sdr/0001.実装対象はXProtocolに従うサーバー.md) |
| WL#9591 | 機能要件 | サーバー側の `caching_sha2_password`、`authentication_string` へのハッシュの保存 (`sha256_password` と同様の手法)、高コストな認証 (平文) | 満たす | [0015](../sdr/0015.認証の方式.md) |
| WL#9591 | 機能要件 | クライアント側の `caching_sha2_password`、`MYSQL_OPT_GET_SERVER_PUBLIC_KEY`、RSA 鍵ペアによるパスワードの交換 | 該当なし | [0001](../sdr/0001.実装対象はXProtocolに従うサーバー.md) |
| WL#9591 | 機能要件 | メモリ上のキャッシュと高速認証 (スクランブル) | 該当なし |  |
| WL#9591 | 機能要件 | 監査クラスと監査イベント、`sha2_cache_cleaner` | 該当なし | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| WL#9591 | 非機能要件 | アップグレード時にアカウントを変更しない | 該当なし |  |
