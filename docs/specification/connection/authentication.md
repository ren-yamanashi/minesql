...
認証に関わる構成要素は 5 つで、セッション ([connection_handler.md](./connection_handler.md)) がこれらを使って認証を進める

- 認証ハンドラ: 認証メカニズム 1 つ分のやり取り (チャレンジの生成、応答の受け取り) を進める
- 認証ハンドラの一覧: メカニズム名と接続の種類 (安全かどうか) から認証ハンドラを作る
- アカウント照合: 資格情報をアカウント情報と突き合わせ、成功したら実行ユーザーを切り替える
- SHA256 パスワードキャッシュ: `SHA256_MEMORY` の照合に使う値を利用者ごとに保持する
- 内部セッションの実行ユーザー: 内部セッションが SQL を実行するときの利用者で、照合の間はシステムユーザー、成功後は認証した利用者になる

```mermaid
flowchart LR
    S["セッション"] -->|"メカニズム名と接続の種類"| L["認証ハンドラの一覧"]
    L -->|"作る"| A["認証ハンドラ"]
    A -->|"資格情報"| V["アカウント照合"]
...
### 認証ハンドラの一覧 (Authentication_container)

- サーバーに 1 つあり、メカニズム名と接続の種類 (安全かどうか) から認証ハンドラを作る
...
- 資格情報を「既定スキーマ \0 ユーザー名 \0 パスワード (または計算値)」に分解し、アカウント情報を取り出して照合する
...
- サーバーに 1 つあり、`SHA256_MEMORY` の照合に使う値 (パスワードの SHA256 の SHA256) を利用者ごとに保持する
...
| コネクションハンドラー: 認証ハンドラとアカウント照合 (この文書) | 認証メカニズムの進行 (salt の生成と応答の受け取り)、資格情報の分解、照合の計算、SHA256 パスワードキャッシュの更新、全体権限の実行ユーザーへの載せ込み |
