...
- MineSQL での仕様
  ...
  - `user` 表の行が持つのは認証文字列と全体権限 (MySQL はさらにプラグイン名、ロック状態、パスワードの期限、TLS の要件を持つ)
...
sequenceDiagram
    ...
    I-->>V: アカウント情報 (認証文字列、全体権限)
    V->>V: 検証器で照合
    V->>I: 実行ユーザーを認証した利用者に切り替え、全体権限を載せる
    V-->>A: 結果
    A-->>S: 成功 / 失敗
    alt 成功
        ...
        S->>S: 3 回目なら認証を打ち切り、接続を閉じる
    end
```

図の段階ごとの補足

- 「検証器で照合」: 検査するのはパスワードの一致だけで、突き合わせる相手と計算は認証メカニズムごとに違う
  - `MYSQL41`: salt に対する応答 (`mysql_native_password` の方式で SHA1 から計算した値) を `mysql.user` の認証文字列と突き合わせる
    - アカウントの認証プラグインは `caching_sha2_password` なので照合は成り立たず、チャレンジを返したうえで access denied になる
  - `SHA256_MEMORY`: 応答 `XOR(SHA256(password), SHA256(SHA256(SHA256(password)) + nonce))` を SHA256 パスワードキャッシュの値 (`SHA256(SHA256(password))`) と突き合わせる
    - キャッシュにその利用者のエントリがなければ失敗する
  - `PLAIN`: 資格情報のパスワード (平文) を `mysql.user` の認証文字列と突き合わせ、成功したら SHA256 パスワードキャッシュに登録する
    - キャッシュに同じ値のエントリがあれば、認証文字列との照合を省いて成功にする
- 「成功」の分岐: `SessionStateChanged` (`CLIENT_ID_ASSIGNED`) の Notice を送り、セッションを利用可能にしてから `AuthenticateOk` を返す
  - 以後、接続は認証した利用者としてコマンドを実行する
- 「失敗」の分岐: 失敗は 1 セッションにつき 3 回まで
  - 3 回目の失敗の `Error` は FATAL になり、サーバーは認証を打ち切って接続を閉じる (途中で別のメカニズムに切り替えて試すことはできる)
  - セッションを閉じた / リセットした後は新しいセッションになるため、回数は 0 から数え直す

図の外側の話として、クライアントがどのメカニズムを使うかは接続の種類で決まる。TLS なしの TCP では `PLAIN` を使えないため、TLS 接続か Unix ソケットで一度 `PLAIN` を通したあとに `SHA256_MEMORY` で認証する
...
| 認証全般 | ハッシュが一致しない場合の失敗 | 満たす | [0015](../sdr/0015.認証の方式.md) |
| 認証全般 | ロックされたアカウントの失敗 | 満たさない | [0015](../sdr/0015.認証の方式.md) |
| 認証全般 | オフラインモードでの `super` 権限のないアカウントの失敗 | 満たさない | [0015](../sdr/0015.認証の方式.md) |
| 認証全般 | パスワードの期限切れによる失敗 | 満たさない | [0015](../sdr/0015.認証の方式.md) |
| 認証全般 | `ssl/tls` オプションが満たされない場合の失敗 | 満たさない | [0015](../sdr/0015.認証の方式.md) |
| 認証全般 | 失敗条件に該当しない場合の成功 | 満たす | [0015](../sdr/0015.認証の方式.md) |
| 認証全般 | 期限切れのアカウントを `Notice` で知らせる | 満たさない | [0015](../sdr/0015.認証の方式.md) |
| 認証資格情報のキャッシュ | 無効化が機能しない場合の失敗 | 該当なし (無効化を止める手段を持たない) |  |
| 認証資格情報のキャッシュ | 安全な接続上の `PLAIN` の成功でキャッシュに保存する | 満たす | [0015](../sdr/0015.認証の方式.md)、[0016](../sdr/0016.TLS接続を実装する.md) |
| 認証資格情報のキャッシュ | 一致するエントリがあれば `PLAIN` は成功する | 満たす | [0015](../sdr/0015.認証の方式.md) |
| 認証資格情報のキャッシュ | `SHA256_MEMORY` はキャッシュのエントリと一致すれば成功する | 満たす | [0015](../sdr/0015.認証の方式.md) |
| 認証資格情報のキャッシュ | `SHA256_MEMORY` は安全でない接続でも安全な接続でも動作する | 満たす | [0015](../sdr/0015.認証の方式.md)、[0016](../sdr/0016.TLS接続を実装する.md) |
| 認証資格情報のキャッシュ | 安全でない接続で tls_option のあるアカウントの失敗 | 満たさない | [0015](../sdr/0015.認証の方式.md) |
| プロトコル実装 | 1 回の失敗で接続を閉じない | 満たす |  |
| プロトコル実装 | 複数回の失敗で接続を閉じる | 満たす |  |
| プロトコル実装 | リセット時に失敗回数を戻す | 満たす |  |
| エッジケース | SQL からキャッシュにアクセスできない | 満たす |  |
| エッジケース | キャッシュを永続化しない | 満たす |  |
| キャッシュの無効化 | `DROP USER` | 満たす | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| キャッシュの無効化 | `SET PASSWORD` | 該当なし (このステートメントを持たない) | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| キャッシュの無効化 | `ALTER USER` | 満たす | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| キャッシュの無効化 | `RENAME USER` | 該当なし (このステートメントを持たない) | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| キャッシュの無効化 | `IDENTIFIED` を伴う `GRANT` | 該当なし (この形を持たない) | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| キャッシュの無効化 | `FLUSH PRIVILEGES` | 該当なし (このステートメントを持たない) | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
...
| `caching_sha2_password` プラグイン | サーバー側の認証プラグイン | 満たす | [0015](../sdr/0015.認証の方式.md) |
| `caching_sha2_password` プラグイン | クライアント側の認証プラグイン | 該当なし (クライアントは作らない) | [0001](../sdr/0001.実装対象はXProtocolに従うサーバー.md) |
| `caching_sha2_password` プラグイン | `authentication_string` へのハッシュの保存 (`sha256_password` と同様の手法) | 満たす | [0015](../sdr/0015.認証の方式.md) |
| `caching_sha2_password` プラグイン | メモリ上のキャッシュ | 該当なし (classic protocol の高速認証を持たない) |  |
| `caching_sha2_password` プラグイン | 高速認証 (スクランブル) | 該当なし (classic protocol の高速認証を持たない) |  |
| `caching_sha2_password` プラグイン | 高コストな認証 (平文) | 満たす | [0015](../sdr/0015.認証の方式.md) |
| `caching_sha2_password` プラグイン | 監査クラス `MYSQL_AUDIT_AUTHENTICATION_CLASS` とそのイベント | 該当なし (監査の枠組みを持たない) | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| `caching_sha2_password` プラグイン | 監査プラグインへの通知 | 該当なし (監査の枠組みを持たない) | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| `caching_sha2_password` プラグイン | 監査プラグイン `sha2_cache_cleaner` | 該当なし (監査の枠組みを持たない) | [0017](../sdr/0017.アカウント管理ステートメントを対象に含める.md) |
| `caching_sha2_password` プラグイン | libmysql のオプション `MYSQL_OPT_GET_SERVER_PUBLIC_KEY` | 該当なし (クライアントは作らない) | [0001](../sdr/0001.実装対象はXProtocolに従うサーバー.md) |
| `caching_sha2_password` プラグイン | 高コストな認証の接続の条件 (ソケット / 共有メモリ / TLS、または RSA 鍵ペアによる交換) | 満たす (TLS または Unix ソケットに限る) | [0016](../sdr/0016.TLS接続を実装する.md) |
| `caching_sha2_password` プラグイン | アップグレード時にアカウントを変更しない | 該当なし (アップグレードを持たない) |  |
