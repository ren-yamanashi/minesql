...
- サーバー: 接続を受け入れるかどうかを決め、受け入れた接続の全体を把握する (X Plugin 内に 1 つ)
- 接続: 1 本の接続の上でフレームを読み書きし、接続の状態を管理する (接続 1 本につき 1 つ)
- セッション: 認証を進め、認証後はその利用者としてコマンドをディスパッチャに引き渡す (接続につき常に 1 つ)

```mermaid
flowchart LR
    S["サーバー"] -->|"受け付けた接続を作り、一覧に持つ"| C["接続"]
    C -->|"常に 1 つ伴い、閉じられたら次を用意する"| N["セッション"]
    N -->|"1 対 1 で、生成と破棄を共にする"| I["内部セッション (SQL 層)"]
