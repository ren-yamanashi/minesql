...
状態と寿命を持ち、コネクションハンドラーの振る舞いの主体となる構成要素は 3 つ
...
flowchart LR
    ...
    N -->|"1 対 1 で、生成と破棄を共にする"| I["内部セッション"]
    N -->|"認証後のコマンドを引き渡す"| D["コマンドディスパッチャ"]
```

### サーバー (Server)

- 全ての接続で共有され、接続を受け入れるかどうかを決め、受け入れた接続の全体を把握する (接続数の上限判定、kill、シャットダウン時の一斉通知はこの把握に基づく)
- 起動から終了までの稼働状態を持ち、終了中は新しい接続を受け入れない

### 接続 (Client)

- 受け付けた接続 1 本につき 1 つ生まれ、接続が閉じると消える
...
- サーバーの内部セッションと 1 対 1 に対応し、生成と破棄のタイミングを共にする
  - 内部セッションとは、実行ユーザーと実行状態 (セッション変数、一時テーブルなど) を持ち、その利用者として SQL を実行する SQL 層側の場 (実装上は THD にあたり、プラグイン向けの srv_session サービスで開閉する)
