...
- 実装時の参照資料 (読む順には含めない): [reference/command_dispatcher_spec.md](./reference/command_dispatcher_spec.md)
  - コマンドディスパッチャの詳細仕様 (種別ごとの振り分け、`StmtExecute` の処理、管理コマンド、Expect ブロック、SQL 層での実行、エラーと深刻度、状態変数)
