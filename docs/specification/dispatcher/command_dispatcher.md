...
- X Plugin のうち、認証済みのセッションが受け取ったリクエストを種別ごとのハンドラに振り分け、SQL 層に実行を委ねて、レスポンスの列を終端まで送り返す部分の仕様
...
- 実装する条件は `no_error` のみ (プロトコルの定義にある他の条件は機能検出向けとドキュメントモデル向けで、対象外、[SDR-0004](../sdr/0004.Expectブロックを実装する.md))
...
- `list_clients` が返すのは同じアカウントの接続だけで、`kill_client` も同じアカウントの接続だけを閉じられる ([SDR-0018](../sdr/0018.権限は全体権限だけを持つ.md))
