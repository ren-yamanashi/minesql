...
- 対応するステートメント (SELECT / INSERT / UPDATE / DELETE / CREATE TABLE / トランザクション制御 / アカウント管理ステートメント / GRANT / REVOKE など) は、MySQL の構文規則にできるだけ従う ([SDR-0007](../sdr/0007.SQLパーサーはMySQLの構文規則に従う.md))
...
- 従わないもの ([SDR-0011](../sdr/0011.文法に含める構文の範囲.md)、[SDR-0013](../sdr/0013.sql_modeの既定値に固定し非推奨と無視される構文は採らない.md))
...
- MineSQL のパーサーは「ステートメントの文字列 → AST」までを担い、MySQL の文脈化に相当する段階を持たない ([SDR-0008](../sdr/0008.パーサーの出力は未解決のクエリブロックと式で文脈化を持たない.md))
  ...
  - パーサーの出口は、AST をステートメント種別ごとの実行コマンド (MySQL の `Sql_cmd` 相当で、`prepare` と `execute` の入口を持つ) に包んだもの ([SDR-0009](../sdr/0009.パーサーの出口は実行コマンドに包む.md))
...
- 実装手段 ([SDR-0010](../sdr/0010.パーサーはgoyaccで生成し字句解析器は手書きにする.md))
  - 字句解析器は手書き (MySQL と同じ)
