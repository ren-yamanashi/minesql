...
- 対応するステートメント (SELECT / INSERT / UPDATE / DELETE / CREATE TABLE / トランザクション制御 / アカウント管理ステートメント / GRANT / REVOKE など) は、MySQL の構文規則にできるだけ従う ([ADR-0007](../adr/0007.SQLパーサーはMySQLの構文規則に従う.md))
