...
- [ページクリーナー](../access/page-cleaner.md)はこのリストの先頭から優先的にフラッシュすることで、[チェックポイント](../access/checkpoint.md)を進める
- ページが最初にダーティーになった時点でリストに追加され、フラッシュ後にリストから除外される
- 既にリストに含まれるページが再度変更されても、リスト内の順序は変わらない (最初にダーティーになった順序を維持する)
