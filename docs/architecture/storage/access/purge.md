...
- パージは [パージ予約 ID](./transaction.md#予約トランザクション-id) で払い出されたトランザクションをパージ自身が静的に 1 つ保持する。パージの初期化時に [トランザクションマネージャ](./transaction.md) 経由で 1 度だけ払い出し、サイクル中はそのトランザクションを使い続ける。サイクルごとに払い出し直すことはしない
- 単一の常駐トランザクションで運用できる根拠は、パージが単一のバックグラウンドゴルーチンから逐次的にしか走らず、同じパージ予約 ID で並行に別のパージサイクルが進行する経路がないため
- パージのバックグラウンド処理が必要とするバッファプール・Redo ログへの参照は、保持しているパージトランザクションから引き出して mini-transaction の払い出しに利用する。パージ自身は個別のリソース参照を引き回さない
- パージトランザクションはアクティブトランザクション一覧に登録せず、MVCC の可視性判定 (ReadView) の参照対象にも含めない。パージはユーザーデータを論理的に書き換える主体ではなく、コミット済みの旧バージョンを物理的に回収する役割しか持たないため、他のトランザクションから「現在実行中のトランザクション」として観測される必要がない。アクティブ集合に含めない設計は、パージ予約 ID 経由の操作がユーザーレコードの `lastTrxId` に出現しないという性質と整合し、ReadView 構築時の `mIds` 集合や `mUpLimitId` 算出に予約 ID を混入させない。同じく MVCC の可視性判定に参加しない他の予約 ID トランザクション (DDL 予約 ID) とも一貫した扱いになる
- パージトランザクションの状態は常に Active のまま固定され、COMMIT / ROLLBACK で Inactive に遷移する経路を持たない。サイクル単位の完了境界に対応する永続化は、サイクル内で払い出した個別の mini-transaction のフラッシュ規律で扱う
...
- パージ専用のトランザクション ID は、ユーザー採番、システム予約 ID、DDL 予約 ID のいずれとも衝突しない値を 1 つ確保する (4 種類の予約 ID の整理は [トランザクション - 予約トランザクション ID](./transaction.md#予約トランザクション-id) を参照)
