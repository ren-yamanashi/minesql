...
1. セカンダリインデックスの B+Tree から `(セカンダリキー, プライマリキー)` を含むレコードを取得する
2. プライマリキーを用いてプライマリインデックスを引き、本体レコードを取得する
3. 本体レコードに対して、上述の [可視性の判定](#可視性の判定) と [Undo チェーン遡及](#undo-チェーン遡及) を行う
4. 可視なバージョンが見つかればその行データを結果として返す。見つからなければ次のセカンダリレコードに進む

可視性はプライマリ側で完結するため、セカンダリレコードの `lastTrxId` / `deleteMark` は判定に利用しない
