...
- セカンダリインデックスのレコードは MVCC 情報 (`lastTrxId`, `rollPtr`) を持たず、`deleteMark` とプライマリキーのみを保持する
- そのためセカンダリインデックス経由の検索では以下の流れで可視性を判定する
  1. セカンダリインデックスの B+Tree から `(セカンダリキー, プライマリキー)` のレコードを取得する
  2. セカンダリレコードの `deleteMark=1` ならスキップして次のセカンダリレコードに進む
  3. プライマリキーを用いてプライマリインデックスを引き、本体レコードを取得する
  4. 本体レコードに対して、上述の[可視性の判定](#可視性の判定)と[Undo チェーン遡及](#undo-チェーン遡及)を行う
  5. 可視なバージョンが見つかればその行データを結果として返す。見つからなければ次のセカンダリレコードに進む
- セカンダリレコードに `lastTrxId` / `rollPtr` を持たない理由
  - 行の最新の MVCC 情報は常にプライマリインデックスに集約されているため
  - セカンダリには「現在の行を指す物理経路」さえあれば十分
  - バージョン遡及はすべてプライマリ側で完結する
