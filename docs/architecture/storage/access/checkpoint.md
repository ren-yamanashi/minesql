...
- バッファプール内のダーティーページの内、最小の Page LSN を取得する
- チェックポイント LSN = (最小の Page LSN) -1
- ダーティーページが存在しない場合 `チェックポイント LSN =　現在の最新 LSN` になる (全変更がディスク上にある)
...
flowchart TD
    ...
    C --> D["チェックポイント LSN = 最小 Page LSN - 1"]
...
1. [ページクリーナー](./page-clearner.md)がバッファプール内のダーティーページの一部をディスクにフラッシュする
2. フラッシュ後、残っているダーティーページの最小 Page LSN から、チェックポイント LSN を算出する
