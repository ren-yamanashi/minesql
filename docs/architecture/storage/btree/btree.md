...
- 新しい B+Tree を作成する際は、まず空の[リーフノード](./node/node-leaf.md)を 1 つ作成し、そのノードをルートノードとして設定する
- また[メタページ](./meta-page.md)も作成し、ルートノードのページ ID をメタページに保存する

## ノードの検索

1. 目的のリーフノードに到達するまで、二分探索で[ブランチノード](./node/node-branch.md)を辿る
2. リーフノード内を二分探索する
...
- 参照
  - [リーフノードのレコードの非キーを更新](./node/node-leaf.md#リーフノード内の特定のレコードの非キーを更新)
