...
- ページ内に複数の可変長データを格納するための手法 (フォーマット)
- B+Tree の各ノード (ブランチ・リーフ) は Slotted Page の形式でデータを保持する
- 外部からは「ページID + スロット番号」という論理的なアドレスで参照される
  - そのため、ページ内でレコードの移動が起きても、スロット内のポインタを更新するだけで済み、外部のインデックスなどを書き換える必要がない
- Slotted Page ではページを以下の 3 つの部分に分けて管理する
  - ヘッダー
  - ポインタ配列
  - セル配列

※ Slotted Page はあくまでもフォーマットに過ぎず、ページ全体が Slotted Page である必要はない
...
上述の [データサイズの変更](#データのサイズ変更) を用いて、該当データのサイズを 0 にする。
...
1. データサイズの変更
   - 上述の [データサイズの変更](#データのサイズ変更) に従って、更新対象のデータのサイズを新しいサイズに変更する
2. データの内容の更新
...
あるスロットページのすべてのスロットを、別のスロットページの末尾に転送する手順は以下の通り

#### 1. 空き容量の確認

該当スロットページのすべてのスロットを転送先のスロットページの末尾に転送するために必要な空き容量が、転送先のスロットページにあるか確認する

#### 2. 該当スロットページのすべてのスロットを転送先のスロットページの末尾に転送する

- 該当スロットページの各スロットのデータを、転送先のスロットページのセル配列の末尾に追加する
- 該当スロットページの各スロットのポインタを、転送先のスロットページのポインタ配列の末尾に追加する
- ヘッダーのスロット数とフリースペースの開始位置 (offset) を更新する

#### 3. 該当スロットページのすべてのスロットを削除する

- 該当スロットページのスロット配列を空にする
