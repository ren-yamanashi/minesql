...
- 他のトランザクションがロックを解放すると Broadcast で全待機者を起床させる。起床したスレッドはラッチを再取得し、自身のロックが付与されたかを確認する
- 付与されていなければ再び Wait で待機する
  - spurious wakeup (条件を満たしてないのに起床する現象) 対策として、必ずループで条件を再チェックする
- タイムアウト時は待機キューから削除してエラーを返す
