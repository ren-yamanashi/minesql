...
- その際に、捨てるページはディスクに書き出す必要がある
  - 捨てるページを選択するアルゴリズムは [LRU](../../about/mysql-lru.md)
...
- ディスクはテーブルごとに作成される (詳細: [ディスクの操作](./disk.md#ディスクの操作)) が、バッファプールはテーブルごとに作成されるわけではないため、結果としてバッファプールには複数のテーブルのページが格納されることが多い
