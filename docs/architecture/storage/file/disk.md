...
- ディスク層はページの採番を行わない
- 新しいページの PageNumber は[ファイル空間管理](../fsp/fsp.md#ページ割り当て)の割り当てで決まり、割り当ての状態はファイル先頭 (page 0) の FSP ヘッダー、extent 記述子、および segment ごとの [inode](../fsp/fseg.md) が保持する。ファイルサイズからの採番導出は行わない
...
- segment に属するページの解放は [file segment (inode) - ページ解放](../fsp/fseg.md#ページ解放) を経由する
...
- 通常運用時は、削除に先立ってファイル削除レコードを Redo に記録し、ディスクへのフラッシュを強制する (リカバリ中のロールバック経路では記録しない。詳細: [クラッシュリカバリ](../access/crash-recovery.md#リカバリ中のロールバック経路と-redo))
