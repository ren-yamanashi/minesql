...
- サーバーは初期ハンドシェイクパケットで `CLIENT_SSL` フラグを通知する
- クライアントが SSL 接続を要求した場合:
  1. クライアントが SSL 接続要求パケット (capability flags のみ) を送信する
  2. サーバーが TCP コネクションを TLS コネクションにアップグレードする
  3. 以降の通信は TLS 上で暗号化される
  4. クライアントがハンドシェイク応答パケットを TLS 上で送信する
- MineSQL ではサーバー起動時に自己署名証明書を自動生成する
- 片方向 TLS (サーバー証明書のみ) を使用
  - クライアント証明書の検証 (mTLS) はサポートしない
- TLS ハンドシェイク完了後の通信はすべて暗号化される
  - Complete Authentication でパスワードを「平文」で送信するとは、MySQL プロトコル上で暗号化せずに送信するという意味であり、TLS による暗号化は適用されている

### 認証

- TLS ハンドシェイク完了後、ハンドシェイク応答パケットの認証データを使って認証が行われる
- 認証には Fast Authentication と Complete Authentication の 2 つのフローがある
...
MineSQL がサポートする Capability Flags は以下の通り
...
- 認証方式は caching_sha2_password のみサポートしている
- サーバーは初期ハンドシェイクパケットに認証プラグイン名と初期認証データ (Nonce) を含めて送信する
- クライアントはサーバーから送られた認証データへの応答をハンドシェイク応答パケットに含めて送信する
- 認証フローの詳細は[認証プラグイン](../../account/authentication-plugin.md)を参照
