...
minesql では INSERT/DELETE/UPDATE などの DML 実行時に "変更行数" のみを記録 (カウント) しておき、一定の閾値を超えたタイミングで、次のクエリ最適化時に統計情報を計算する方法を採用している (採用理由は [ADR/テーブル統計情報の収集方法](../adr/1.テーブル統計情報の収集方法.md) を参照)
