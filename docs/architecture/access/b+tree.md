...
- データ (key-value のペア) を持つ
  ...
  - また value は固定長ではなく可変長の場合があり、複数の可変長のデータを格納するためには、ブロックにおける「それぞれのデータの開始位置 (オフセット)」とサイズを管理する必要がある
    - これを実現するために、Slotted Page という手法を用いる (詳細は後述)
- 葉ノード同士はポインタで連結されている
  - このため範囲検索が高速に行える
...
- ヘッダー
  - ページ全体のメタデータを管理する
    - 例: ページ内のデータ数、ポインタ配列の開始位置、セル配列の開始位置など

- ポインタ配列
  - 各データの開始位置 (オフセット) とサイズを管理する
  - ページの左端に位置する
  - 左から右へ成長する (新しいポインタが追加される)

- セル配列
  - 実際のデータを格納する
  - ページの右端に位置し、データは右から左へ追加される
  - 内部ノードは key と子ノードへのポインタを持つので、セルはポインタになる
