...
- テーブルごとに個別のディスクファイルを持つ
  ...
  - 各テーブルは独自の FileId を持ち、対応するディスクファイルに格納される
...
DiskManager は個別のディスクファイルを管理する。各テーブルごとに 1 つの DiskManager が存在し、そのテーブルのページの読み書きを担当する。
...
- ページ ID を採番する
  ...
  - 採番された PageId は `{FileId: fileId, PageNumber: pageNumber}` の形式
...
- バリデーション:
  ...
  - PageId の FileId がこの DiskManager の fileId と一致することを確認
