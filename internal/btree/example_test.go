...
func setupExample() (*btree.BPlusTree, *storage.BufferPool, func()) {
	...
	dm, err := storage.NewDisk(fileId, filepath.Join(tmpDir, "example.db"))
	if err != nil {
		panic(err)
	}
...
func printAll(bp *storage.BufferPool, tree *btree.BPlusTree) {
