...
func TestCommitTrx(t *testing.T) {
	t.Run("コミット後もデータが永続化されている", func(t *testing.T) {
		...
		iter, err := tbl.Search(h.BufferPool, readTrxId, h.LockMgr, transaction.RecordSearchModeStart{})
		assert.NoError(t, err)
...
func TestRollbackTrx(t *testing.T) {
	t.Run("ロールバックで Insert が取り消される", func(t *testing.T) {
		...
		assert.NoError(t, err)
		iter, err := tbl.Search(h.BufferPool, 0, lock.NewManager(5000), transaction.RecordSearchModeStart{})
		assert.NoError(t, err)
		_, ok, err := iter.Next()
		assert.NoError(t, err)
		assert.False(t, ok)
