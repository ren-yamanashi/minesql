...
type LRU struct {
	...
	nodeMap  map[buftype.BufferId]*lruNode // BufferId → ノードの参照
...
func NewLRU(size int) *LRU {
	policy := &LRU{
		nodeMap: make(map[buftype.BufferId]*lruNode, size),
		maxNew:  size * 5 / 8,
	}
	// 全スロットを OldSublist に追加 (初期状態では全て未使用)
	for i := range size {
		node := &lruNode{
			bufferId: buftype.BufferId(i),
			isOld:    true,
			isUnused: true,
		}
		policy.nodeMap[buftype.BufferId(i)] = node
