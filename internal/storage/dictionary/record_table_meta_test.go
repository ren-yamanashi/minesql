...
func TestTableMetaRecordName(t *testing.T) {
	t.Run("コンストラクタで指定した名前を返す", func(t *testing.T) {
		// GIVEN
		tr := NewTableMetaRecord("users", page.NewId(page.FileId(1), page.PageNumber(0)), 3)
...
func TestTableMetaRecordColumnCount(t *testing.T) {
...
func TestTableMetaRecordEncode(t *testing.T) {
	...
	t.Run("エンコードした結果をデコードすると元のデータに戻る", func(t *testing.T) {
		// GIVEN
		original := NewTableMetaRecord("users", page.NewId(page.FileId(1), page.PageNumber(0)), 3)
	...
	t.Run("カラム数が 0 の場合も正しくエンコード・デコードできる", func(t *testing.T) {
		// GIVEN
		original := NewTableMetaRecord("empty_table", page.NewId(page.FileId(2), page.PageNumber(0)), 0)
	...
	t.Run("長いテーブル名でも正しくエンコード・デコードできる", func(t *testing.T) {
		// GIVEN
		original := NewTableMetaRecord("very_long_table_name_for_testing", page.NewId(page.FileId(3), page.PageNumber(0)), 10)
	...
	t.Run("MetaPageId のページ番号が 0 の場合も正しくエンコード・デコードできる", func(t *testing.T) {
		// GIVEN
		original := NewTableMetaRecord("t", page.NewId(page.FileId(0), page.PageNumber(0)), 1)
...
func TestDecodeTableMetaRecord(t *testing.T) {
	t.Run("エンコード済みレコードから MetaPageId を復元できる", func(t *testing.T) {
		// GIVEN
		tr := NewTableMetaRecord("orders", page.NewId(page.FileId(42), page.PageNumber(0)), 5)
		record := tr.Encode()
	...
	t.Run("エンコード済みレコードからテーブル名とカラム数を復元できる", func(t *testing.T) {
		// GIVEN
		tr := NewTableMetaRecord("products", page.NewId(page.FileId(1), page.PageNumber(0)), 7)
		record := tr.Encode()
