...
func TestIndexMetaRecordMetaPageId(t *testing.T) {
	t.Run("コンストラクタで指定したメタページ ID を返す", func(t *testing.T) {
		// GIVEN
		metaPageId := page.NewId(page.FileId(1), page.PageNumber(0))
		ir := NewIndexMetaRecord(page.FileId(1), IndexId(10), "idx_name", IndexTypeNonUnique, 2, metaPageId)
...
func TestIndexMetaRecordEncode(t *testing.T) {
	...
	t.Run("MetaPageId をエンコード・デコードできる", func(t *testing.T) {
		...
		original := NewIndexMetaRecord(page.FileId(1), IndexId(1), "idx_test", IndexTypePrimary, 1, metaPageId)
