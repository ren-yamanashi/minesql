...
func TestPageLSN(t *testing.T) {
	t.Run("SetPageLSN で書き込んだ値を PageLSN で読み取れる", func(t *testing.T) {
		// GIVEN
		data := make([]byte, 128)
		sp := NewSlottedPage(data)
		sp.Initialize()
		...
		// THEN
		assert.Equal(t, uint32(42), sp.PageLSN())
	})

	t.Run("Initialize 後の PageLSN は 0", func(t *testing.T) {
		// GIVEN
		data := make([]byte, 128)
		sp := NewSlottedPage(data)
	...
	t.Run("PageLSN の変更が他のヘッダーフィールドに影響しない", func(t *testing.T) {
		// GIVEN
		data := make([]byte, 128)
		sp := NewSlottedPage(data)
		sp.Initialize()
		sp.Insert(0, []byte("hello"))
