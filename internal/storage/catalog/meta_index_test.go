...
func TestIndexMetaInsert(t *testing.T) {
	t.Run("インデックスメタデータを挿入できる", func(t *testing.T) {
		...
		// WHEN
		err := im.Insert(IndexRecord{FileId: page.FileId(1), Name: PrimaryIndexName, IndexId: IndexId(1), IndexType: IndexTypePrimary, NumOfCol: 1, MetaPageId: page.NewPageId(page.FileId(1), page.PageNumber(0))})
	...
	t.Run("同じ FileId + インデックス名の重複挿入は ErrDuplicateKey を返す", func(t *testing.T) {
		// GIVEN
		im := setupTestIndexMeta(t)
		_ = im.Insert(IndexRecord{FileId: page.FileId(1), Name: PrimaryIndexName, IndexId: IndexId(1), IndexType: IndexTypePrimary, NumOfCol: 1, MetaPageId: page.NewPageId(page.FileId(1), page.PageNumber(0))})

		// WHEN
		err := im.Insert(IndexRecord{FileId: page.FileId(1), Name: PrimaryIndexName, IndexId: IndexId(2), IndexType: IndexTypePrimary, NumOfCol: 1, MetaPageId: page.NewPageId(page.FileId(1), page.PageNumber(0))})
	...
	t.Run("異なるインデックス名であれば同じテーブルに複数挿入できる", func(t *testing.T) {
		...
		// WHEN
		err := im.Insert(IndexRecord{FileId: page.FileId(1), Name: "idx_email", IndexId: IndexId(2), IndexType: IndexTypeUnique, NumOfCol: 1, MetaPageId: page.NewPageId(page.FileId(1), page.PageNumber(0))})
...
func TestIndexMetaSearch(t *testing.T) {
	t.Run("SearchModeStart で全件スキャンできる", func(t *testing.T) {
		// GIVEN
		im := setupTestIndexMeta(t)
		_ = im.Insert(IndexRecord{FileId: page.FileId(1), Name: "PRIMARY", IndexId: IndexId(1), IndexType: IndexTypePrimary, NumOfCol: 1, MetaPageId: page.NewPageId(page.FileId(1), page.PageNumber(0))})
		_ = im.Insert(IndexRecord{FileId: page.FileId(1), Name: "idx_name", IndexId: IndexId(2), IndexType: IndexTypeNonUnique, NumOfCol: 2, MetaPageId: page.NewPageId(page.FileId(1), page.PageNumber(0))})
